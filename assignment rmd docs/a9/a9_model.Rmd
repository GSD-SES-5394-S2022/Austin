...
Capital Metro prices them at the cost of 36 rides in that timeframe).
...
rail riders are more likely to use monthly passes? Based on literally nothing other 
than a hunch, (no data, anecdotes, or anecdata) I would guess that a "Typical Commuter 
Rail Rider" might(?) be more likely to have a monthly pass.

UPDATE: some data
https://www.ctps.org/dv/mbtasurvey2018/2015_2017_Passenger_Survey_Final_Report.pdf
At least in Boston, commuter rail riders are slightly more likely to use monthly
passes when paying fares, which if the same applies to Austin, then that could explain
a lower cost per transit trip.
...
#HBW - Let's try: 1, -1.5, -2.5, -2, -3
#HBW - What if all zeroes?
#HBW - Try 1, -1, 1, 2, -1
#HBW - Closer - try 0.5, -0.5, 2, 5, -0.5
#HBW - Tweak - 0.7, -0.5, 2, 5, 0
#HBW - Tweak - 0.85, -0.75, 2, 5, 0.25
#HBW - Close! - 0.9, -0.75, 2, 4.5, 0.25
#HBW - One more nudge - 0.9, -0.7, 2, 4.75, 0.15
#HBW - Bring up SOV - 0.95, -0.7, 2, 4.75, 0.15
#HBW - other attemps made it worse, sticking with this
...
#HBO - First correction - -.5, 0, -3, 0, -2
#HBO - Fix transit - -.5, 0, 0, 0.5, -2
#HBO - Transit overcorrected - -.5, 0, -1.5, 0.5, -2
#HBO - Nudge down SOV and Transit - -.75, -0.1, -1.75, 0.5, -1.5
#HBO - Up SOV -.6, -0.1, -1.6, 0.5, -1.75
#HBO - Up HOV slightly - -.6, -0.2, -1.6, 0.5, -1.75
#HBO - Final Nudge - -.5, 0, -1.5, 0.6, -1.75

SOV_const_HBO = -0.5
HOV_const_HBO = 0
transit_const_HBO = -1.5
walk_const_HBO = 0.6
bike_const_HBO = -1.75


#NHB - Initial run: -0.5, 0, -4.5, -2.2, -5.5
#NHB - Use the HBO constants - -0.5, 0, -1.5, 0.6, -1.75
#NHB - Tweak - -0.5, -0.5, -1.5, 0.6, -2.5
#NHB - Tweak - -0.8, -0.5, -1.5, 0.6, -4

SOV_const_NHB = -0.8
HOV_const_NHB = -0.5
transit_const_NHB = -1.5
walk_const_NHB = 0.6
bike_const_NHB = -4
...
```{r model constants}
mode_constants <- data.frame(trip_type = c("HBW","HBO","NHB"),
                             SOV = c(SOV_const_HBW,SOV_const_HBO, SOV_const_NHB),
                             HOV = c(HOV_const_HBW, HOV_const_HBO, HOV_const_NHB),
                             transit = c(transit_const_HBW, transit_const_HBO, transit_const_NHB),
                             walk = c(walk_const_HBW, walk_const_HBO, walk_const_NHB),
                             bike = c(bike_const_HBW, bike_const_HBO, bike_const_NHB))
```
...
modeled_mode_by_purpose_3 <- tibble(
  purpose = "NHB_model_1", 
...
model_compare <- rbind(mode_by_purpose, modeled_mode_by_purpose_2,
                       modeled_mode_by_purpose_1, modeled_mode_by_purpose_3)

model_compare %>% filter(str_starts(purpose, "NHB")) %>%
  select(purpose, pct_SOV, pct_HOV, pct_transit, pct_walk, pct_bike)
```

We're going to calibrate constants for each trip type separately.
Initial iteration:
For HBW - undercounting bike, walk, and transit, and even HOV, and overcounting SOV.
For HBO - nudge up SOV, bring transit, walk, and bike way up

```{r}
mode_constants %>% filter(trip_type == "NHB")
```

To see calibration history, look at the line comments in the code chunk w/ the
constants.
...
# Applying to Alternative Condition
...
And to display our comparison

```{r}
alt_modeled_mode_by_purpose_1 <- tibble(
  purpose = "HBW_model_alt", 
...
alt_modeled_mode_by_purpose_3 <- tibble(
  purpose = "NHB_model_alt", 
...
ex_alt_compare <- rbind(modeled_mode_by_purpose_2,
                       modeled_mode_by_purpose_1, modeled_mode_by_purpose_3,
                       alt_modeled_mode_by_purpose_2,
                       alt_modeled_mode_by_purpose_1,
                       alt_modeled_mode_by_purpose_3)
...
## Writing our data out
```{r write complete table}

#this is a large file, it won't sync to github

skims %>%
  write_csv(here("large datasets",
                 "travel skims with utilities and probabilities",
                 "skims_mode_numbers_with_utilities_probabilities_ex.csv"))

skims_alt %>%
  write_csv(here("large datasets",
                 "travel skims with utilities and probabilities",
                 "skims_mode_numbers_with_utilities_probabilities_alt.csv"))

```


```{r write only the costs and numbers}
skims_prev <- read_csv(here("existing",
                            "data",
                            "all_skims_ex.csv")) %>%
  mutate(fromTo = paste0(fromId,toId),
         fromId = as.character(fromId),
         toId = as.character(toId))

skims_prev_alt <- read_csv(here("alternative",
                                "data",
                                "all_skims_alt.csv")) %>%
  mutate(fromTo = paste0(fromId,toId),
         fromId = as.character(fromId),
...
new_stuff_ex <- skims %>%
  mutate(fromTo = paste0(fromId,toId)) %>%
  select('fromTo','drive_cost','transit_cost','carpool_cost',
         'n_SOV_HBW','n_HOV_HBW','n_transit_HBW','n_walk_HBW','n_bike_HBW',
         'n_SOV_HBO','n_HOV_HBO','n_transit_HBO','n_walk_HBO','n_bike_HBO',
         'n_SOV_NHB','n_HOV_NHB','n_transit_NHB','n_walk_NHB','n_bike_NHB')
  

new_stuff_alt <- skims_alt %>%
  mutate(fromTo = paste0(fromId,toId)) %>%
  select('fromTo','drive_cost','transit_cost','carpool_cost',
         'n_SOV_HBW','n_HOV_HBW','n_transit_HBW','n_walk_HBW','n_bike_HBW',
         'n_SOV_HBO','n_HOV_HBO','n_transit_HBO','n_walk_HBO','n_bike_HBO',
...
skims_merged <- full_join(skims_prev, new_stuff_ex) %>%
  select(-c(fromTo))

skims_alt_merged <- full_join(skims_prev_alt, new_stuff_alt) %>%
  select(-c(fromTo))

skims_merged %>%
  write_csv(here("existing",
                 "data",
                 "all_skims_mode_numbers_ex.csv"))

skims_alt_merged %>%
  write_csv(here("alternative",
                 "data",
...
```{r}
pie_data <- read.csv(here("assignment rmd docs", "a9", "pie_data.csv")) %>%
  mutate(mode = c("Bike", "SOV", "HOV", "Transit", "Walk"),
         HBO_ch = (HBO_alt-HBO_ex)*100/HBO_ex,
         HBW_ch = (HBW_alt-HBW_ex)*100/HBW_ex,
         NHB_ch = (NHB_alt-NHB_ex)*100/NHB_ex)
  
# Compute percentages
pie_data$fraction_ex <- pie_data$Existing / sum(pie_data$Existing)

# Compute the cumulative percentages (top of each rectangle)
pie_data$ymax_ex <- cumsum(pie_data$fraction_ex)

# Compute the bottom of each rectangle
pie_data$ymin_ex <- c(0, head(pie_data$ymax, n=-1))
...
# Label radius
pie_data <- pie_data %>%
  mutate(radius_ex = ifelse(Existing > 5, 3.5, 4),
         radius_alt = ifelse(Alternative > 5, 3.5, 4))

# Make the plot
ggplot(pie_data, aes(ymax=ymax_ex, ymin=ymin_ex, xmax=4, xmin=3, fill=mode)) +
  geom_rect() +
  geom_label(x=pie_data$radius_ex, aes(y=labelPosition, 
                                       label=scales::percent(fraction_ex, accuracy=0.01)), size=3, show.legend = FALSE) +
...
ggsave(here("images", "a9", "ex_modeshare_all.png"))
```

```{r}
# Compute percentages
pie_data$fraction_alt <- pie_data$Alternative / sum(pie_data$Alternative)

# Compute the cumulative percentages (top of each rectangle)
pie_data$ymax_alt <- cumsum(pie_data$fraction_ex)

# Compute the bottom of each rectangle
pie_data$ymin_alt <- c(0, head(pie_data$ymax_alt, n=-1))

# Compute label position
pie_data$labelPosition <- (pie_data$ymax_alt + pie_data$ymin_alt) / 2

# Label radius
pie_data <- pie_data %>%
  mutate(radius_alt = ifelse(Existing > 5, 3.5, 4),
         radius_alt = ifelse(Alternative > 5, 3.5, 4))

# Make the plot
ggplot(pie_data, aes(ymax=ymax_alt, ymin=ymin_alt, xmax=4, xmin=3, fill=mode)) +
  geom_rect() +
  geom_label(x=pie_data$radius_alt, aes(y=labelPosition, 
                                        label=scales::percent(fraction_alt, accuracy=0.01)), size=3, show.legend = FALSE) +
  scale_fill_brewer(palette="Oranges") +
  coord_polar(theta="y") +
  xlim(c(2, 4)) +
  theme_void() +
...
ggsave(here("images", "a9", "alt_modeshare_all.png"))
```

```{r}
# Compute percentages
pie_data$fraction_ex <- pie_data$HBO_ex / sum(pie_data$HBO_ex)

# Compute the cumulative percentages (top of each rectangle)
pie_data$ymax_ex <- cumsum(pie_data$fraction_ex)

# Compute the bottom of each rectangle
pie_data$ymin_ex <- c(0, head(pie_data$ymax_ex, n=-1))

# Compute label position
pie_data$labelPosition <- (pie_data$ymax_ex + pie_data$ymin_ex) / 2

# Label radius
pie_data <- pie_data %>%
  mutate(radius_ex = ifelse(HBO_ex*100 > 5, 3.5, 4),
         radius_alt = ifelse(Alternative > 5, 3.5, 4))
...
ggsave(here("images", "a9", "ex_modeshare_HBO.png"))
```

```{r}
# Compute percentages
pie_data$fraction_ex <- pie_data$HBW_ex / sum(pie_data$HBW_ex)
...
# Label radius
pie_data <- pie_data %>%
  mutate(radius_ex = ifelse(HBW_ex*100 > 5|mode=="Walk", 3.5, 4),
         radius_alt = ifelse(Alternative > 5, 3.5, 4))
...
ggsave(here("images", "a9", "ex_modeshare_HBW.png"))
```

```{r}
# Compute percentages
pie_data$fraction_ex <- pie_data$NHB_ex / sum(pie_data$NHB_ex)
...
# Label radius
pie_data <- pie_data %>%
  mutate(radius_ex = ifelse(NHB_ex*100 > 5|mode=="Walk", 3.5, 4),
         radius_alt = ifelse(Alternative > 5, 3.5, 4))
...
ggsave(here("images", "a9", "ex_modeshare_NHB.png"))
```
```{r}
# Compute percentages
pie_data$fraction_ex <- pie_data$HBW_ex / sum(pie_data$HBW_ex)
...
ggsave(here("images", "a9", "excomp_modeshare_HBW.png"))
```

```{r}
# Compute percentages
pie_data$fraction_alt <- pie_data$HBW_alt / sum(pie_data$HBW_alt)

# Compute the cumulative percentages (top of each rectangle)
pie_data$ymax_alt <- cumsum(pie_data$fraction_alt)
...
ggsave(here("images", "a9", "altcomp_modeshare_HBW.png"))
```

```{r}
zones <- st_read(here("zones", "boundaries.geojson"))

dt_skim_ex <- skims_merged %>%
  filter(toId == "48453000700"|toId == "48453001100") %>%
  nest_by(fromId) %>%
  mutate(transit_HBW_ex = sum(data$n_transit_HBW)/sum(data$HBW_flow),
         bike_HBW_ex = sum(data$n_bike_HBW)/sum(data$HBW_flow),
         walk_HBW_ex = sum(data$n_walk_HBW)/sum(data$HBW_flow),
         sov_HBW_ex = sum(data$n_SOV_HBW)/sum(data$HBW_flow))

zones_ex <- zones %>%
  left_join(dt_skim_ex,
            by = c("GEOID" = "fromId"))

dt_skim_alt <- skims_alt_merged %>%
  filter(toId == "48453000700"|toId == "48453001100") %>%
  nest_by(fromId) %>%
  mutate(transit_HBW_alt = sum(data$n_transit_HBW)/sum(data$HBW_flow),
         bike_HBW_alt = sum(data$n_bike_HBW)/sum(data$HBW_flow),
         walk_HBW_alt = sum(data$n_walk_HBW)/sum(data$HBW_flow),
         sov_HBW_alt = sum(data$n_SOV_HBW)/sum(data$HBW_flow))

dt_skim_ex <- dt_skim_ex %>%
  select(fromId, transit_HBW_ex, bike_HBW_ex, walk_HBW_ex, sov_HBW_ex)

zones_alt <- zones %>%
  left_join(dt_skim_alt,
            by = c("GEOID" = "fromId")) %>%
  left_join(dt_skim_ex,
            by = c("GEOID" = "fromId")) %>%
  mutate(transit_HBW_ch = (transit_HBW_alt - transit_HBW_ex)/transit_HBW_ex,
         bike_HBW_ch = ifelse((bike_HBW_alt - bike_HBW_ex)/bike_HBW_ex>.50, .51,(bike_HBW_alt - bike_HBW_ex)/bike_HBW_ex),
         walk_HBW_ch = (walk_HBW_alt - walk_HBW_ex)/walk_HBW_ex,
         sov_HBW_ch = (sov_HBW_alt - sov_HBW_ex)/sov_HBW_ex)
```

```{r}
# ggplot(zones_ex) +
#   geom_sf(aes(fill = transit_HBW_ex),
#           color = NA) +
#   theme_void()
```
...
ggplot(zones_alt) +
  geom_sf(aes(fill = transit_HBW_ch),
          color = NA) +
...
ggsave(here("images", "a9", "transit_share_HBW.png"))
```

```{r}
pal2 <- pnw_palette(name = "Sunset2", 
                           n = 5, 
                           type = "continuous")

ggplot(zones_alt) +
  geom_sf(aes(fill = bike_HBW_ch),
          color = NA) +
...
ggsave(here("images", "a9", "bike_share_HBW.png"))
```

```{r}
pal3 <- pnw_palette(name = "Starfish", 
                           n = 5, 
                           type = "continuous")

ggplot(zones_alt) +
  geom_sf(aes(fill = walk_HBW_ch),
          color = NA) +
...
ggsave(here("images", "a9", "walk_share_HBW.png"))
```

```{r}
pal4 <- pnw_palette(name = "Moth", 
                           n = 5, 
                           type = "continuous")

ggplot(zones_alt) +
  geom_sf(aes(fill = sov_HBW_ch),
          color = NA) +
  scale_fill_gradientn(colours = pal4,
                       labels = scales::percent) +
  labs(fill = "% Decrease in \nSOV Share \n(Home-based \nwork trips)") +
  theme_void()

ggsave(here("images", "a9", "sov_share_HBW.png"))
```