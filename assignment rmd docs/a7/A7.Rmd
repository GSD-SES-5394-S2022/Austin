...
export_summs(HBO_model1,
  ...
  quick_rtf(file = "HBO Full Model.rtf")
```
...
export_summs(HBO_model1, HBO_model2,
  ...
  quick_rtf(file = "HBO both models.rtf")
```
...
export_summs(HBW_model1, HBW_model2,
             ...
             error_format = "(p = {p.value})",
             model.names = c("Full model", "Reduced model")) %>%
  quick_rtf(file = "HBW model compare.rtf")
```
...
export_summs(NHB_model1, NHB_model2,
             ...
             error_format = "(p = {p.value})",
             model.names = c("Full model", "Reduced model")) %>%
  quick_rtf(file = "NHB model compare.rtf")
```
...
trips_map <- full_join(zone_sf, existing_zones) %>%
  select(GEOID, geometry, hbo_prod, hbw_prod, nhb_prod,
...
hbw_p_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$hbw_prod/500))

hbo_p_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$hbo_prod/500))

nhb_p_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$nhb_prod/500))

hbw_a_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$hbw_attr/500))

hbo_a_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$hbo_attr/500))

nhb_a_pts <- st_sample(trips_map,
                       size = ceiling(trips_map$nhb_attr/500))
```
...
prod_dens_map <- ggplot(zone_sf) +
  geom_sf(color = "white") +
  geom_sf(data = trips_prod_pts,
          aes(color = trip_type),
          alpha = 0.3,
          size = 0.1) +
