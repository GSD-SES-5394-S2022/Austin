...
The purpose of this R Markdown document is to assemble a dataset for building our travel model. First, we import ACS data with population characteristics. Next, we download employment information for the MSA and join it with the census data. Additional variables are calculated from this. Finally, the spatial information is separated from the data, and they are both uploaded to Github. The second half of this document generates data visualizations used in our report.
...
bas_map + 
  theme_void() +
  scale_fill_viridis_c(name = "Basic Jobs")
```
...
ret_map + 
  theme_void() +
  scale_fill_gradientn(name = "Retail Jobs",
                       colors = pnw_contin2)
...
serv_map + 
  theme_void() +
  scale_fill_gradientn(name = "Service Jobs",
                       colors = pnw_contin5)
